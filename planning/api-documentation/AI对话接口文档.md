...
| 对话锁服务不可用 | `503` | 对话锁服务暂不可用，请稍后再试 | Redis 锁服务异常，后端不会降级为无锁执行 |
| 对话不存在 | `404` | 对话不存在 | — |
...
- **认证**：登录用户
- **并发说明**：同一对话同一时间只允许一个请求执行；AI 生成或上下文压缩期间再次发送会返回 `409`。如果对话锁服务不可用，返回 `503`，后端不会降级为无锁执行。
...
- **显示规则**：默认返回用户消息、有正文且不携带 `tool_calls` 的 AI 回复，以及 compact 提示消息。工具原始结果、系统消息、携带 `tool_calls` 的 AI 中间决策消息默认隐藏，即使该中间消息本身有说明性正文。compact summary 的真实摘要内容不会通过该接口返回，只返回占位文案 `已自动压缩上下文`。
