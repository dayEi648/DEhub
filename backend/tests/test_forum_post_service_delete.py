...
def test_delete_post_cascades_reply_comments_and_embedded_images(
    ...
    mock_comment_crud.delete_comment_likes_by_comment_ids.assert_called_once_with(
        db, [301, 302], auto_commit=False
    )
    mock_comment_crud.delete_comments_by_ids.assert_called_once_with(
        db, [301, 302], auto_commit=False
    )
    assert mock_cleanup_file.call_count == 3
    mock_post_crud.delete_post.assert_called_once_with(db, 100, auto_commit=False)
...
@patch("app.services.forum_post_service.convert_oss_url_to_file_path")
@patch("app.services.forum_post_service.OssCleanupService.delete_file_after_commit_sync")
def test_delete_post_commit_failure_rolls_back_and_skips_oss_cleanup(
    mock_cleanup_file,
    mock_convert_path,
    mock_extract_urls,
    mock_is_manager,
