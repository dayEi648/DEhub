...
async function loadTags() {
  ...
  try {
    tags.value = await getTopTags('instrument', 10)
...
<template>
  <section class="discover-section">
    <div class="section-header-row">
      <h2 class="section-title">按乐器浏览</h2>
    </div>

    <div v-loading="tagsLoading" class="tag-filter">
      <div v-for="tag in tags" :key="tag" class="tag-item" :class="{ active: activeTag === tag }" @click="selectTag(tag)">
        {{ tag }}
      </div>
