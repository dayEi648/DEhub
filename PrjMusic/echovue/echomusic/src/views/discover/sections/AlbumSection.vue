...
import { getAlbumPage, getHomeRecommendAlbums } from '@/api/discover'
import type { AlbumVO, PageDataVo } from '@/types/album'

const router = useRouter()

const recommendLoading = ref(false)
const recommendList = ref<AlbumVO[]>([])

const loading = ref(false)
const albumData = ref<PageDataVo<AlbumVO>>({ total: 0, records: [] })
const pageNum = ref(1)
...
async function loadRecommend() {
  ...
  try {
    recommendList.value = await getHomeRecommendAlbums()
  } catch {
    recommendList.value = []
  } finally {
...
async function loadData() {
  loading.value = true
  try {
    albumData.value = await getAlbumPage({
    ...
    albumData.value = { total: 0, records: [] }
  } finally {
    loading.value = false
  }
}
  ...
  <section class="discover-section">
    <div class="recommend-block">
      ...
      <div v-else class="recommend-grid">
        <div v-for="al in recommendList" :key="al.id" class="media-card" @click="goAlbumDetail(al.id)">
          ...
          <div class="media-meta">{{ joinNames(al.authorNames) }} · 收藏{{ formatCount(al.collectCount) }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="all-block">
      ...
      <div v-loading="loading" class="card-grid">
        <div v-for="al in albumData.records" :key="al.id" class="media-card" @click="goAlbumDetail(al.id)">
      ...
      <div v-if="albumData.total > 0" class="pagination-wrap">
        <el-pagination v-model:current-page="pageNum" :page-size="pageSize" :total="albumData.total" layout="prev, pager, next" @current-change="handlePageChange" />
      </div>
      <el-empty v-else-if="!loading" description="暂无专辑" />
    </div>
  </section>
</template>
